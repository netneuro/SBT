function varargout = defence_sigstar(groups, stats, effect_size, nosort)

    % Edited by Zahra Mor on April 2021, to add the effect sizes near asterisks.
    % The original method is sigstar.m as in the sigstar package: https://github.com/raacampbell/sigstar/
    ...
    for ii=1:length(groups)
        ...
        H(ii,:)=makeSignificanceBar(xlocs(ii,:), thisY, stats(ii), effect_size(ii));
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x, y, p, effect_size)
    ...
    H(2)=text(mean(x(:)),starY,stars,...
        ...
        'Tag','sigstar_stars');
    text(mean(x(:) + 0.1), starY, num2str(effect_size, '%.2f'), 'BackgroundColor', 'w', 'FontSize', 9);
